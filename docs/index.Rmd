...
|
| [Random Replacement](https://cran.r-project.org/web/packages/missCompare/missCompare.pdf) replaces missing values with random instances from the the data. This method is typically used as a baseline method from which other imputations methods are compared with, but may not be appropriate to be applied in practice due to its inability to take into account any patterns or relationships in the data that may exist between the variable with missing values and other variables in the dataset. This can lead to imputed values that do not reflect the true values of the missing data driving unstable and inaccurate estimates of relationships between variable.
|
